...
def rasterization(
    ...
    ar_proj_results = fully_fused_projection(
        means,
        covars,
        quats,
        scales,
...
def _sonar_rasterization(
    means: Tensor,  # [N, 3]
    quats: Tensor,  # [N, 4]
    scales: Tensor,  # [N, 3]
    opacities: Tensor,  # [N]
    sat_probability: Tensor,  # [N]
    colors: Tensor,  # [(C,) N, D] or [(C,) N, K, 3]
    viewmats: Tensor,  # [C, 4, 4]
    Ks: Tensor,  # [C, 3, 3]
    width: int,
    height: int,
    max_range: float,
    near_plane: float = 0.01, # -1e10, # For sonar ortho projection # default: 0.01
    far_plane: float = 1e10,
    eps2d: float = 0.3,
    sh_degree: Optional[int] = None,
    tile_size: int = 16,
    ...
    from gsplat.cuda._torch_impl_sonar import (
        _rasterize_to_sonar_pixels,
        _compute_per_gaussian_transmittance,
    ...
    ############################################################Prepare AE Tensors############################################################
    perm_mat = torch.tensor([[0, 1, 0], 
                             [0, 0, 1], 
                             [1, 0, 0]]).to(rae_means)

    aer_means = (perm_mat @ rae_means.T).T
    
    #perform symmetric op on rae_covars
    aer_covars = torch.zeros_like(rae_covars)
    aer_covars[:, 0] = rae_covars[:, 3]
    aer_covars[:, 1] = rae_covars[:, 4]
    aer_covars[:, 2] = rae_covars[:, 1]
    aer_covars[:, 3] = rae_covars[:, 5]
    aer_covars[:, 4] = rae_covars[:, 2]
    aer_covars[:, 5] = rae_covars[:, 0]

    ae_height = int(Ks[0, 1, 1] * (far_plane - near_plane))
    ae_width = height

    ae_Ks = torch.zeros_like(Ks)
    ae_Ks[:, 1, 1] = Ks[:, 1, 1]
    ae_Ks[:, 1, 2] = ae_height / 2.0
    ae_Ks[:, 0, 0] = Ks[:, 1, 1]
    ae_Ks[:, 0, 2] = Ks[:, 1, 2]
    ae_Ks[:, 2, 2] = 1.0


    ae_radii, ae_means2d, ae_depths, ae_conics, ae_compensations = fully_fused_projection(
        means=aer_means,
        covars=aer_covars,
        viewmats=i_view_mats,
        Ks=ae_Ks,
        quats=quats,
        scales=scales,
        width=ae_width,
    ...
    opacities = opacities.repeat(C, 1)  # [C, N]
    sat_probability = sat_probability.repeat(C, 1)  # [C, N]

    camera_ids, gaussian_ids = None, None

    if ae_compensations is not None:
        opacities = opacities * ae_compensations

    # Identify intersecting tiles
    ae_tile_width = math.ceil(ae_width / float(tile_size))
    ae_tile_height = math.ceil(ae_height / float(tile_size))
    
    ae_tiles_per_gauss, ae_isect_ids, ae_flatten_ids = isect_tiles(
        ae_means2d,
        ae_radii,
        ae_depths,
        tile_size,
        ...
        camera_ids=camera_ids,
        gaussian_ids=gaussian_ids,
        sort=True # Default to True. Change to False make sonarGS fail.
    )
    ae_isect_offsets = isect_offset_encode(ae_isect_ids, C, ae_tile_width, ae_tile_height)

    ae_per_gaussian_transmittance = _compute_per_gaussian_transmittance(
        means2d=ae_means2d,
        conics=ae_conics,
        opacities=opacities,
        image_width=ae_width,
        image_height=ae_height,
        tile_size=tile_size,
        isect_offsets=ae_isect_offsets,
        flatten_ids=ae_flatten_ids,
        batch_per_iter=batch_per_iter,
    )
    ...
    ############################################################AR Rasterization############################################################
    ar_radii, ar_means2d, ar_depths, ar_conics, ar_compensations  = fully_fused_projection(
        rae_means,
        rae_covars, 
        quats,
        scales,
    ...
    if ar_compensations is not None:
        opacities = opacities * ar_compensations

    depths_isect_tiles = torch.zeros_like(ar_depths)
    ...
    if distributed:
        if packed:
            ...
            collected_splits = all_to_all_int32(world_size, cnts, device=device)
            (radii,) = all_to_all_tensor_list(
                world_size, [ar_radii], cnts, output_splits=collected_splits
            )
            (means2d, depths, conics, opacities, colors) = all_to_all_tensor_list(
                world_size,
                [ar_means2d, ar_depths, ar_conics, opacities, colors],
                cnts,
                output_splits=collected_splits,
            )
            ...
            (means2d, depths, conics, opacities, colors) = all_to_all_tensor_list(
                world_size,
                [
                    ar_means2d.flatten(0, 1),
                    ar_depths.flatten(0, 1),
                    ar_conics.flatten(0, 1),
                    opacities.flatten(0, 1),
                    colors.flatten(0, 1),
                ],
                splits=[C_i * N for C_i in C_world],
    ...
    render_alphas, saturated_returns, opacity_returns, all_gs_ids = _rasterize_to_sonar_pixels(
        means2d=ar_means2d,
        conics=ar_conics,
        colors=colors,
        opacities=opacities,
        sonar_max_range=max_range,
        per_gaussian_transmittance=ae_per_gaussian_transmittance,
    ...
    if torch.any(torch.isnan(saturated_returns)):
        print("render_alphas has nan values")
        print(render_alphas)
        print(opacity_returns)
        print(saturated_returns)
        print(all_gs_ids)
        
    meta = {
        "unsaturated_returns": render_alphas,
        "opacity_returns": opacity_returns,
        "camera_ids": camera_ids,
        "gaussian_ids": all_gs_ids,
        "radii": ar_radii,
        "means2d": ar_means2d,
        "depths": ar_depths,
        "conics": ar_conics,
        "colors": colors,
        "opacities": opacities,
        "transmittances": ae_per_gaussian_transmittance,
        "tile_width": tile_width,
        "tile_height": tile_height,
        "tiles_per_gauss": tiles_per_gauss,
        "isect_ids": isect_ids,
