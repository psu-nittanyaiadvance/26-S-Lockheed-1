    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if step >= cfg.elevate_start_step and step <= cfg.elevate_end_step and \
                ...
                # Decide on rank 0 whether to do elevation sampling, broadcast decision
                should_elevate = pixels_for_loss.sum() > 0
                if world_size > 1:
                    import torch.distributed as dist
                    flag = torch.tensor([1 if (should_elevate and world_rank == 0) else 0], device=device)
                    dist.broadcast(flag, src=0)
                    should_elevate = flag.item() > 0

                if should_elevate:
                    desc += "Elevation Sampling"

                    if world_size == 1 or world_rank == 0:
                        # Only rank 0 computes sampling (random ops would desync ranks)
