...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if gt_density_h is not None and gt_density_w is not None:
            ZL = (l2_loss(z_density_h, gt_density_h) * dataset.h_res + l2_loss(z_density_w, gt_density_w) * dataset.w_res) / (dataset.h_res + dataset.w_res)
            if opt.depth_loss:
                loss += ZL
        loss.backward()
...
def prepare_output_and_logger(args):    
    ...
    # Append timestamp subfolder so multiple runs don't overwrite each other
